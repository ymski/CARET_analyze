...
class TestLttng:
    ...
    def test_compose_path_beginning_records(self, mocker):
        data_mock = mocker.Mock(spec=Ros2DataModel)
        mocker.patch.object(Lttng, '_parse_lttng_data',
                            return_value=(data_mock, {}, 0, 1))
        ...
        records_mock = mocker.Mock(spec=RecordsInterface)
        mocker.patch.object(records_mock, 'clone', return_value=records_mock)
        mocker.patch.object(records_source_mock, 'path_beginning_records',
                            records_mock)
        assert lttng.compose_path_beginning_records() == records_mock

    def test_compose_path_end_records(self, mocker):
        data_mock = mocker.Mock(spec=Ros2DataModel)
        mocker.patch.object(Lttng, '_parse_lttng_data',
                            return_value=(data_mock, {}, 0, 1))
        ...
        mocker.patch.object(records_source_mock, 'callback_records',
                            records_mock)
        assert lttng.compose_path_end_records() == records_mock
