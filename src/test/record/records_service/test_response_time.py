...
class TestResponseHistogram:
    ...
    def test_invalid_vs_valid(self):
        ...
        invalid_response = ResponseTime(invalid_records)
        valid_response = ResponseTime(valid_records)

        invalid_hist, invalid_latency = invalid_response.to_histogram(1)
        valid_hist, valid_latency = valid_response.to_histogram(1)

        assert list(invalid_hist) == list(valid_hist)
        assert list(invalid_latency) == list(valid_latency)

        invalid_hist, invalid_latency = invalid_response.to_best_case_histogram(1)
        valid_hist, valid_latency = valid_response.to_best_case_histogram(1)
        assert list(invalid_hist) == list(valid_hist)
        assert list(invalid_latency) == list(valid_latency)

        invalid_hist, invalid_latency = invalid_response.to_worst_case_histogram(1)
        valid_hist, valid_latency = valid_response.to_worst_case_histogram(1)
        assert list(invalid_hist) == list(valid_hist)
        assert list(invalid_latency) == list(valid_latency)
...
class TestResponseTimeseries:
    ...
    def test_empty(self):
        ...
        start_ts, latency = response.to_best_case_timeseries()
        assert list(start_ts) == []
        assert list(latency) == []

        start_ts, latency = response.to_worst_case_timeseries()
        assert list(start_ts) == []
        assert list(latency) == []
    ...
    def test_double_flow_case(self):
        ...
        start_ts, latency = response.to_best_case_timeseries()
        assert list(start_ts) == [2]
        assert list(latency) == [1]

        start_ts, latency = response.to_worst_case_timeseries()
        assert list(start_ts) == [0]
        assert list(latency) == [3]
    ...
    def test_triple_flow_case(self):
        ...
        start_ts, latency = response.to_best_case_timeseries()
        assert list(start_ts) == [2, 10]
        assert list(latency) == [1, 1]

        start_ts, latency = response.to_worst_case_timeseries()
        assert list(start_ts) == [0, 2]
        assert list(latency) == [3, 9]

    def test_invalid_vs_valid_case(self):
        invalid_records_raw = [
            {'start': 0, 'end': 1},
            {'start': 6, 'end': 5},
            {'start': 10, 'end': 11},
        ...
        invalid_start_ts, invalid_latency = invalid_response.to_best_case_timeseries()
        valid_start_ts, valid_latency = valid_response.to_best_case_timeseries()

        assert list(invalid_start_ts) == list(valid_start_ts)
        assert list(invalid_latency) == list(valid_latency)

        invalid_start_ts, invalid_latency = invalid_response.to_worst_case_timeseries()
        valid_start_ts, valid_latency = valid_response.to_worst_case_timeseries()

        assert list(invalid_start_ts) == list(valid_start_ts)
        assert list(invalid_latency) == list(valid_latency)

        invalid_records = invalid_response.to_best_case_records()
        valid_records = valid_response.to_best_case_records()

        assert to_dict(invalid_records) == to_dict(valid_records)

        invalid_records = invalid_response.to_worst_with_external_latency_case_records()
        valid_records = valid_response.to_worst_with_external_latency_case_records()
